...
export class FormComponent implements OnInit {
  ...
  checkValueChanges(): void {
    ...
    this.filterForm?.get('connectionId')?.valueChanges.pipe(distinctUntilChanged()) .subscribe(val => {
      if (this.filterForm) {
        if (val) {
    ...
    this.filterForm?.get('reportType')?.valueChanges.pipe(distinctUntilChanged()) .subscribe(val => {
      if (this.filterForm) {
        if (val) {
