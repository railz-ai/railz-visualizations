...
export default function Customization() {
    ...
    return (
        <div className="App">
            <Header description={'This page shows you the customizable options that can be passed to the SDK.'}>
                <div className="md:grid md:grid-cols-3 md:gap-6">
                    <div className="md:col-span-1 shadow p-4">
                        <Form setFilter={submitFilter} setAuthentication={submitAuthentication} setError={setError} options={options} setOptions={submitOptions}/>
                    </div>
                    <div className="mt-5 md:mt-0 md:col-span-2">
                        {!token && 'No Token, submit your authentication details'}
                        {token && isEmpty(filter) && 'No Filter, submit your filter details'}
                        {token && !isEmpty(filter) && <Visualizations configuration={{token, debug: true, environment: 'local'}}
                                                                      filter={filter as any} options={options}/>}
