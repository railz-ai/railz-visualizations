    ...
    return <>
        {showCode && <pre className="language-html bg-black text-white mt-4 p-4 rounded-xl overflow-hidden">
              <code className="language-html">
                  {"<" + displayValue + " configuration={" + JSON.stringify({token: configuration.token.slice(0, 20) + '...'}, null, '\t') + "} filter={" + JSON.stringify(formatCodeFilter(filter) as any, null, '\t') + `${options ? '} options={' + JSON.stringify(options, null, '\t') : ''}} />`}
              </code>
            </pre>}
    </>
}
...
const DefaultComponent = ({configuration, filter, options, showCode}: ChartProps) => {
    return <div>
        <RailzVisualizations configuration={configuration} filter={filter as RVFilter} options={options}/>
        <Code configuration={configuration} filter={filter} options={options} showCode={showCode} displayValue="RailzVisualizations"/>
    </div>
}
    ...
    return <div className="mt-5 md:grid md:grid-cols-1 md:gap-1">
        ...
        {filter.reportType === 'all' && [RVReportTypes.BILLS, RVReportTypes.INVOICES, RVReportTypes.BALANCE_SHEET, RVReportTypes.INCOME_STATEMENTS, RVReportTypes.CASHFLOW_STATEMENTS].map((reportType) =>
            <div className="col-span-1"
                 key={reportType}>
                <DefaultComponent
                    configuration={configuration} filter={{...filter, reportType} as Filter} options={options} showCode={showCode}/>
            </div>)}
        {filter.reportType === 'balanceSheets' && <div>
            ...
            <RailzStatementsChart configuration={configuration} filter={filter as RVFilterFrequency} options={options}/>
            <Code configuration={configuration} filter={filter} options={options} showCode={showCode}
        ...
        {filter.reportType === 'bills' && <div>
            ...
            <RailzTransactionsControl configuration={configuration} filter={filter as RVFilterDate} options={options}/>
            <Code configuration={configuration} filter={filter} options={options} showCode={showCode}
                  displayValue="RailzTransactionsControl"/>
        </div>}
        {!['balanceSheets', 'bills', 'all'].includes(filter.reportType) && <DefaultComponent
            configuration={configuration} filter={filter} options={options} showCode={showCode}/>}
    </div>
}
