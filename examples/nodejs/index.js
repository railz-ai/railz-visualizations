...
*/
const AUTH_URL =
  process.env.RAILZ_AUTH_URL || "https://auth.railz.ai/getAccess";
...
app.get("/authenticate", (request, response) => {
  ...
  const encodedString = Buffer.from(`${CLIENT_ID}:${CLIENT_SECRET}`).toString(
    "base64"
  );
  axios
    ...
    .then((res) => {
      ...
      response
        .status(200)
        .json({ success: true, access_token: res.data.access_token });
    })
    .catch((error) => {
      console.error(error);
